...
- **i18n dogfoods `yapyak` directly** — no wrapper hook:
...
- **Design tokens live in `src/style.css`** as CSS custom properties (`--bg`, `--text`, `--mint`, `--space-*`, `--radius-*`, etc.). CSS Modules conventions otherwise per [[css-modules]].
