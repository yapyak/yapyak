...
export function anthropic(options: AnthropicOptions): Translator {
  ...
  async function batch(requests: TranslateRequest[]): Promise<string[]> {
    ...
    const response = await fetch(endpoint, {
      body: JSON.stringify({
        max_tokens: Math.max(1024, requests.length * 256),
...
function parseBatchResponse(text: string, expectedLength: number): string[] {
  ...
  try {
    ...
    throw new Error(
      `yapyak/translators/anthropic: failed to parse batch JSON response: ${(cause as Error).message}\n${cleaned.slice(0, 200)}`,
    );
  }
  if (!Array.isArray(parsed)) {
    throw new Error(
      'yapyak/translators/anthropic: batch response was not a JSON array',
    );
  }
