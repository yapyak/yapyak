...
- [`b847d55`](https://github.com/yapyak/yapyak/commit/b847d552600c9d8211d17ed070cba1723edb62ce) Thanks [@qwuide](https://github.com/qwuide)! - A directive expression holding an HTML entity no longer corrupts the emitted component. `@vue/compiler-sfc` reports the expression decoded but its position raw, so `:title="t('Tom &amp; Jerry')"` rewrote the wrong span and produced a broken template. The processor now maps the decoded expression back to the raw source one run at a time.

- [`89b6562`](https://github.com/yapyak/yapyak/commit/89b656229a047667539ce4906b8041f0d0b5bc2c) Thanks [@qwuide](https://github.com/qwuide)! - A directive expression holding an HTML entity without its trailing semicolon no longer corrupts the emitted component. Vue decodes legacy references like `&lt` and numeric references like `&[#38](https://github.com/yapyak/yapyak/issues/38)` without the semicolon, so `:title="t('Tom &amp Jerry')"` rewrote the wrong span and produced a broken template.
