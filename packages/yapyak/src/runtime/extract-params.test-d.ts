...
test('no placeholders → unknown', () => {
  expectTypeOf<ExtractParams<'Save changes'>>().toEqualTypeOf<unknown>();
});

test('single simple placeholder', () => {
  expectTypeOf<ExtractParams<'Hello, {name}!'>>().toEqualTypeOf<{
    name: string | number;
  }>();
});

test('multiple simple placeholders', () => {
  expectTypeOf<{ a: 'x'; b: 1 }>().toMatchTypeOf<ExtractParams<'{a} {b}'>>();
});

test('ICU plural — count typed as number', () => {
  type Result =
    ExtractParams<'You have {count, plural, one {# message} other {# messages}}'>;
...
test('ICU selectordinal — typed as number', () => {
  type Result = ExtractParams<'{n, selectordinal, one {1st} other {nth}}'>;
...
test('ICU select — theme typed as string', () => {
  type Result =
    ExtractParams<'{theme, select, dark {Dark mode} other {Light mode}}'>;
...
test('ICU number — amount typed as number', () => {
  type Result = ExtractParams<'Price: {amount, number, currency EUR}'>;
...
test('ICU date — when typed as Date | number', () => {
  type Result = ExtractParams<'Updated: {when, date, long}'>;
...
test('ICU time — when typed as Date | number', () => {
  type Result = ExtractParams<'At: {when, time, short}'>;
  expectTypeOf<{ when: Date }>().toMatchTypeOf<Result>();
