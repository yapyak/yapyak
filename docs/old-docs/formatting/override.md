...
Forces a locale for one call or for the rest of the module's life. `format.in('sv')` returns a [`Format`](./overview.md#the-format-type) that behaves exactly like the unscoped one — same methods — except every result comes out in Swedish regardless of who is looking.

```ts
import { format } from 'yapyak';

format.in('sv').number(1234.5);                                      // '1 234,5' even if the active locale is 'en'
format.in('en').number(199, { style: 'currency', currency: 'EUR' }); // '€199.00' even if the active locale is 'sv'
```
...
sv.number(199, { style: 'currency', currency: 'SEK' });      // '199,00 kr'
sv.dateTime(new Date(), { dateStyle: 'long' });              // '12 november 2025'
```
