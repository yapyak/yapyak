...
A locale in yapyak is the BCP 47 name of one of the JSON files in your `localesDir`. `'sv'` for `locales/sv.json`, `'en-GB'` for `locales/en-GB.json`.
...
2. **Detected from the environment** when [`detectUserLocale`](/guide/getting-started/configuration#detectuserlocale) is enabled. The source depends on where the code runs: the `Accept-Language` request header on the server (under the [SSR adapter](/guide/getting-started/installation)), or `navigator.languages` in the browser at runtime initialization. The detected value is matched against your configured locales.
